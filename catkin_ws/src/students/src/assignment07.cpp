/*
 * @HOME BASIC TASKS - UdelaR 2023
 * ASSIGNMENT 07 - LOCALIZATION BY PARTICLE FILTERS
 *
 * Instructions:
...
{
    std::cout << "ASSIGNMENT 07 - " << FULL_NAME << std::endl;
    ros::init(argc, argv, "assignment07");
    ros::NodeHandle n("~");
    ros::Rate loop(20);
    ...
    while(ros::ok())
    {
        ...
        if(check_displacement(robot_odom, delta_pose))
        {
             ...
             * TODO:
             *
             * Move all particles a displacement given by delta_pose by calling the move_particles function.
